...
namespace WebApplication3.ChessLogic
{
    public class Player
    {
        ...
        public Player(Game Game, Colors color)
        {
            ...
            IsInCheck = false;
            KingCoordinates = new Coordinates { Row = Color == Colors.White ? 0 : MainConstants.BoardHeight - 1, Col = 4 };
        ...
        {
            if (Game.PlayState.Type != PlayStateTypes.Running) return new List<Coordinates>();

            var selectedPiece = Game.Board[coordinates.Row, coordinates.Col]?.Piece;

            if (selectedPiece == null) return new List<Coordinates>();

            var type = selectedPiece.Type;

            if (attackers.Count > 1 && type != PieceTypes.King) return new List<Coordinates>();

            var legalMoves = selectedPiece.GetLegalMoves(Game);

            if (attackers.Any())
            {
                var attackingCoordinatesPath = new List<Coordinates>();
        ...
        public List<Coordinates> SelectPiece(Coordinates coordinates)
        {
            ...
            if (clickedPiece == null) return new List<Coordinates>();
        ...
        public void UpdateGame(Game updatedGame)
        {
            ...
            var newAttackers = CoordinatesHelper.GetSquareAttackers(KingCoordinates, Color, Game.Board);
        ...
        public Piece? GetPossibleKingAttacker(Piece selectedPiece)
        {
            ...
            var closestPieceInOppositeDirection = CoordinatesHelper.GetPieceByCoordinates(
                gameState,
                CoordinatesHelper.SearchForPieceCoordinates(
                    gameState,
                    selectedPiece.Coordinates,
